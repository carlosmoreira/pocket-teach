...
// Hijack the reply so Nest/Fastify won't serialize a response. Every write is
...
export function startSse(reply: FastifyReply, req: FastifyRequest): SseStream {
  ...
  // Hijack bypasses the CORS reply hook, so echo the origin here to mirror the
  // reflect-any-origin policy on the streamed response.
